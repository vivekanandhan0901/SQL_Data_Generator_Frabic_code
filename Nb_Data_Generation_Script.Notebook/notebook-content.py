...
class Config:
    ...
    CUSTOMERS_TO_CREATE = 5000
    SUPPLIERS_TO_CREATE = 200
...
def seed_products(spark: SparkSession) -> int:
    ...
    for col_name in ['isBestSeller', 'boughtInLastMonth']:
        if col_name in products_df.columns:
            products_df = products_df.drop(col_name)

    # Safely cast numeric columns that exist
    for col_name in ['price', 'listPrice', 'category_id']:
        if col_name in products_df.columns:
            products_df = products_df.withColumn(col_name, F.col(col_name).cast("double"))

    # Drop rows missing required fields
    required_cols = ['price', 'category_id']
    products_df = products_df.dropna(subset=[col for col in required_cols if col in products_df.columns])

    # Fill missing float fields (like listPrice) with 0.0, only if they exist
    fill_values = {}
    for col_name in ['listPrice']:
        if col_name in products_df.columns:
            fill_values[col_name] = 0.0
    products_df = products_df.fillna(fill_values)
...
def seed_customers(spark: SparkSession, count, batch_size):
    """
    Seeds the 'customers' table with fake data in batches using JDBC insert.
    ...
    Returns:
        List[int]: List of inserted customer IDs (if fetched).
    """
    ...
    # Get max ID to return newly inserted IDs
    initial_df = spark.read.jdbc(url=jdbc_url, table="customers", properties=connection_properties)
    initial_max_ID = initial_df.agg({"id": "max"}).collect()[0][0] or 0
    ...
    while inserted < count:
        ...
        inserted += len(batch)
        print(f"Prepared {inserted} customers...")
    ...
    # Insert into customers table using JDBC
    customer_df.write.jdbc(url=jdbc_url, table="customers", mode="append", properties=connection_properties)
    print(f"Inserted total {len(all_data)} customer records into the database.")

    # Optionally fetch new IDs
    final_df = spark.read.jdbc(url=jdbc_url, table="customers", properties=connection_properties)
    new_ids = final_df.filter(col("id") > initial_max_ID).select("id").rdd.flatMap(lambda x: x).collect()
...
def seed_orders(spark, customers_to_seed, days, generate_sequentially, max_products_per_order, some_value, max_orders_per_customer):
    cache_all_products(spark)  # Cache product list first
    ...
    order_date = Config.ORDER_GENERATION_DATE
    yr, mn, dy = map(int, order_date.split("-"))

    # Get max order ID to simulate order ID
    max_id_df = spark.read \
        ...
        .option("driver", "com.microsoft.sqlserver.jdbc.SQLServerDriver") \
        .load()
    doc_order_id = max_id_df.collect()[0]["max_id"] + 1
    ...
    for customer_id in customer_ids:
        ...
        while True:
            ...
            order_sum = sum(item["listPrice"] * item["qty"] for item in order["products"])
            order_number = f"{order_date}/{doc_order_id}"
            ...
            for item in order["products"]:
                item_total = item["listPrice"] * item["qty"]
                order_items.append((
                    doc_order_id,
                    item["product"],
                    item["qty"],
...
def seed_suppliers(spark: SparkSession):
    """
    Seeds fake suppliers using Spark and inserts into SQL Server via JDBC.
    
    Args:
        spark: SparkSession
    
    Returns:
        int: Number of inserted suppliers
    """
    ...
    existing_suppliers = existing_df.select("companyName").rdd.flatMap(lambda x: x).collect()
    existing_set = set(existing_suppliers)

    suppliers_to_create = Config.SUPPLIERS_TO_CREATE
    created_suppliers = []

    for _ in range(1, suppliers_to_create):
        contact = generate_new_customer()
        company = contact["company"]
        if company in existing_set:
            continue  # Skip if already exists

        address = f"{contact['address']}/n{contact['city']}/n{contact['state']}/n{contact['zip']}"
    ...
    suppliers_df = spark.createDataFrame(created_suppliers, schema=supplier_schema)
    ...
    print(f"Inserted {len(created_suppliers)} new suppliers.")
    return len(created_suppliers) + len(existing_suppliers)
...
def seed_purchase_orders(spark: SparkSession, po_count, days_back, product_filter="All Sold"):
    ...
    print("Generate your PO records and use for insert. See other refactored seed functions for pattern.")
    ...
    try:
        ...
        if "seed_customers" in scenario:
            ...
            if existing is None or existing < Config.CUSTOMERS_TO_CREATE:
                if test_mode:
                    seed_customers(spark, Config.CUSTOMERS_TO_CREATE // 100, Config.DEFAULT_BATCH_SIZE)

                else:
                    seed_customers(spark, Config.CUSTOMERS_TO_CREATE, Config.DEFAULT_BATCH_SIZE)
